...
@pytest.mark.asyncio
async def test_prompt_suite_generation_uses_extra_body_and_exact_tg():
    client = LLMClient(
        "http://example.test/v1",
        "EMPTY",
        "model",
    ...
    result = await client.run_prompt_suite_generation(
        session,
        name="code_python",
    ...
    assert payload["min_tokens"] == 12
    assert payload["ignore_eos"] is True
    assert result.predicted_n == 12
