...
| `prompt_size`    | integer | User-facing `--pp` for synthetic requests; `0` for prompt-suite requests. The actual server count is reported on `request_end`. |
| `response_size`  | integer | User-facing `--tg`, or `--suite-max-tokens` for a prompt suite.        |
| `context_size`   | integer | User-facing `--depth`; `0` for a prompt suite.                         |
| `concurrency`    | integer | Concurrency level for this batch; currently `1` for a prompt suite.   |
| `run_index`      | integer | Zero-based run index within the current benchmark cell or suite pass. |
| `target_label`   | string  | Prompt name for a prompt-suite request; otherwise empty in v1.         |
...
During synthetic benchmarks, `latency_measured` fires once before any
`request_start` events. Prompt suites do not measure or emit latency.
...
### Non-streaming prompt suites

Prompt suites use non-streaming responses so backend timing fields remain
available. Each prompt therefore emits `request_start` followed by `request_end`,
but does not emit `request_first_response`, `request_first_token`, or `tokens`.
Because decode duration is not observable separately, `request_end.decode_seconds`
is `0.0`; prompt-suite reports retain the complete request wall time.
...
- Phase labels (`PREFILL` / `DECODE` / `DONE`). For streaming requests,
  phases are derivable: a request is `PREFILL` between `request_start` and
  `request_first_token`, `DECODE` between `request_first_token` and
  `request_end`, and `DONE`/`ERROR` after `request_end`. Non-streaming prompt
  suites expose only request start and completion.
